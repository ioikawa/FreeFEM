...
[^1]:
GMRES法とMINRES法は「残差が単調減少（非増加）である」ことが数学的に保証されている．
CG法やBiCGSTAB法は時には残差が爆発的に増加して計算が破綻する恐れがあるのに対して，
GMRES法・MINRES法は計算が破綻する可能性は（理論的には）ない．
...
2つメッシュ $\mathcal{T}_h, \mathcal{T}_{h'}$ に対する有限要素解 $u_h, u_{h'}$を計算する．
